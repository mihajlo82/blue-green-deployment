...
1. **Create two EC2 instances** (Blue and Green) in the same VPC.
2. **Create two Target Groups** (Blue and Green) pointing to respective EC2 instances.
3. **Create an Application Load Balancer** and attach both target groups.
4. **Set up an ALB listener** on port 80/443.
5. **Configure security groups** to allow SSH from GitHub Actions and traffic from ALB.
6. **Optional:** Assign Elastic IPs for consistent public access to EC2 instances.
...
2. GitHub Actions automatically deploys to idle EC2.
3. ALB listener switches traffic to the updated environment.
4. Users are routed seamlessly to the new version.
...
**Mihajlo Dimitric** – DevOps / Full Stack Developer  
